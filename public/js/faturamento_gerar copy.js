$(document).ready(function () {
    ...
    /*  function construirTabelaFaturamentoEdicao(gruposFazenda) {
          ...
          for (const nomeFazenda in gruposFazenda) {
              ...
              notasDaFazenda.forEach((nota, notaIndex) => {
                  ...
                  const obsHtml = nota.observacao
                      ? `<small class="d-block text-muted fst-italic">Obs: ${nota.observacao}</small>` : '';
                  const tabelaId = `tabela-faturamento-${nomeFazenda.replace(/\s+/g, '-')}-${notaIndex}`;
  
                  html += `<table class="table table-sm table-bordered table-hover mb-3 w-100 tabela-faturamento" id="${tabelaId}">
                                <thead class="table-light">
                                    <tr>
          ...
          // Inicializa DataTables responsivo para cada tabela de nota
    ...
    /*   function construirTabelaFaturamentoEdicao(gruposFazenda) {
           ...
           for (const nomeFazenda in gruposFazenda) {
               ...
               notasDaFazenda.forEach((nota, notaIndex) => {
                   ...
                   html += `
                   <div class="mb-2">
                       <strong class="text-primary">Cliente:</strong> ${nota.cliente_nome}
                       <span class="ms-2">(Pedido: ${nota.numero_pedido})</span>
                       <div class="mt-1">${condPagHtml}</div>
                       ${obsHtml}
               ...
               html += `<table class="table table-sm table-bordered mt-2 w-100">
                       <tr class="table-dark fw-bold">
                           <td class="text-end" style="width: 40%;">TOTAL FAZENDA (${nomeFazenda}):</td>
                           <td class="text-end" style="width: 10%;">${formatarNumeroBrasileiro(subTotalFazendaCaixas)}</td>
                           <td class="text-end" style="width: 10%;">${formatarNumeroBrasileiro(subTotalFazendaQuilos)}</td>
                           <td style="width: 15%;"></td>
                           <td class="text-end" style="width: 15%;">${formatCurrency(subTotalFazendaValor)}</td>
           ...
           html += `<table class="table table-bordered mt-4 w-100">
                   <tr class="table-primary fw-bolder">
                       ...
                       <td class="text-end" style="width: 10%;">${formatarNumeroBrasileiro(granTotalQuilos)}</td>
                       <td style="width: 15%;"></td>
                       <td class="text-end" style="width: 15%;">${formatCurrency(granTotalValor)}</td>
                       <td style="width: 10%;"></td>
                   </tr>
               </table>`;
    ...
    /* function construirTabelaFaturamentoEdicao(gruposFazenda) {
         ...
         for (const nomeFazenda in gruposFazenda) {
             ...
             <table class="table table-sm table-bordered table-hover mb-3 w-100 tabela-faturamento" id="${tabelaId}">
                 ...
                 // ✅ Corrigido: subtotal com 7 colunas exatas
                 html += `<tr class="table-light fw-bold" style="font-style: italic;">
                 <td colspan="2" class="text-end">Subtotal (Nota):</td>
                 <td class="text-end">${formatarNumeroBrasileiro(subTotalNotaCaixas)}</td>
                 <td class="text-end">${formatarNumeroBrasileiro(subTotalNotaQuilos)}</td>
     ...
     *
     * O erro de 'Uncaught TypeError' foi corrigido movendo a linha
     * de subtotal da nota (que usa colspan) do <tbody> para o <tfoot>.
     * O DataTables ignora o <tfoot> durante sua indexação principal,
     * resolvendo o problema de estrutura inconsistente de células.
     *
     * @param {object} gruposFazenda - Dados de faturamento agrupados por nome da fazenda.
     */
    function construirTabelaFaturamentoEdicao(gruposFazenda) {
        // Variável global (assumida) para o container de destino
        const $container = $('#container-resumo-faturamento'); // Assumindo que o ID é este ou similar
        ...
        // Função auxiliar para formatação de moeda (assumida como existente)
        const formatCurrency = (value) => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        // Função auxiliar para formatação de números (assumida como existente)
        const formatarNumeroBrasileiro = (value, decimals = 3) => {
        ...
        for (const nomeFazenda in gruposFazenda) {
            ...
            <table class="table table-sm table-bordered table-hover mb-3 w-100 tabela-faturamento" id="${tabelaId}">
                ...
                <tbody>`; // Apenas as linhas de item vão no <tbody>
                ...
                // CORREÇÃO CRÍTICA: Mover subtotal para <tfoot>
                html += `</tbody>
                <tfoot>
            ...
            // Tabela de TOTAL FAZENDA (6 colunas)
            html += `<table class="table table-sm table-bordered mt-2 w-100">
            <tr class="table-dark fw-bold">
                <td class="text-end" style="width: 40%;">TOTAL FAZENDA (${nomeFazenda}):</td>
                <td class="text-end" style="width: 10%;">${formatarNumeroBrasileiro(subTotalFazendaCaixas)}</td>
        ...
        // Tabela de TOTAL GERAL (6 colunas)
        html += `<table class="table table-bordered mt-4 w-100">
        <tr class="table-primary fw-bolder">
            <td class="text-end" style="width: 40%;">TOTAL GERAL DO RESUMO:</td>
            <td class="text-end" style="width: 10%;">${formatarNumeroBrasileiro(granTotalCaixas)}</td>
        ...
        $('.tabela-faturamento').each(function () {
            ...
            $tabela.DataTable({
                ...
                // Não é necessário usar o footerCallback, pois o <tfoot> já foi movido
            });
        });
    }
