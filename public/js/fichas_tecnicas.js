// /public/js/fichas_tecnicas.js - VERSÃO FINAL COM CROPPER.JS FUNCIONAL
...
$(document).ready(function () {
    ...
    if ($('#tabela-fichas-tecnicas').length) {
        $('#tabela-fichas-tecnicas').DataTable({
            ...
            columns: [
                { data: "ficha_id", render: data => String(data).padStart(4, '0') },
                { data: "prod_descricao" },
                { data: "prod_marca" },
                { data: "prod_ncm" },
                { data: "ficha_data_modificacao", render: data => data ? new Date(data).toLocaleString('pt-BR') : '' },
                {
                    data: "ficha_id", orderable: false, className: "text-center",
                    render: function (data, type, row) {
                        let btnEditar = `<a href="index.php?page=ficha_tecnica_detalhes&id=${data}" class="btn btn-warning btn-sm me-1" title="Editar"><i class="fas fa-pencil-alt"></i></a>`;
                        let btnCopiar = `<button class="btn btn-info btn-sm btn-copiar-ficha me-1" data-id="${data}" title="Copiar"><i class="fas fa-copy"></i></button>`;
                        let btnExcluir = `<button class="btn btn-danger btn-sm btn-excluir-ficha" data-id="${data}" data-nome="${row.prod_descricao}" title="Excluir"><i class="fas fa-trash"></i></button>`;
        ...
        $('#tabela-fichas-tecnicas').on('click', '.btn-copiar-ficha', function () {
            ...
            $.post('ajax_router.php?action=getFichaTecnicaCompleta', { ficha_id: fichaId, csrf_token: csrfToken }, (response) => {
                if (response.success) {
        ...
        $('#tabela-fichas-tecnicas').on('click', '.btn-excluir-ficha', function () {
            ...
            Swal.fire({
                title: 'Tem certeza?', text: `Deseja realmente excluir a ficha técnica do produto "${produtoNome}"?`, icon: 'warning',
                showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'Cancelar', confirmButtonText: 'Sim, excluir!'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.post('ajax_router.php?action=excluirFichaTecnica', { ficha_id: fichaId, csrf_token: csrfToken }, (response) => {
                        if (response.success) {
    ...
    if ($('#main-title').length) {
        ...
        const select2ProdutoConfig = {
            ...
            theme: "bootstrap-5",
            ajax: { url: "ajax_router.php?action=getProdutosSemFichaTecnica", dataType: 'json', delay: 250, data: p => ({ term: p.term }), processResults: d => d }
        };
        ...
        function preencherFormularioFicha(fichaData, isCopy = false) {
            ...
            if (header.ficha_fabricante_id && header.fabricante_nome) {
                $('#ficha_fabricante_id').append(new Option(header.fabricante_nome, header.ficha_fabricante_id, true, true)).trigger('change');
            }
            if (header.ficha_produto_id && header.produto_nome) {
                ...
                $selectProduto.html(new Option(header.produto_nome, header.ficha_produto_id, true, true)).trigger('change');
            }
            ...
            $.get(`ajax_router.php?action=listarCriteriosFicha&ficha_id=${fichaId}`, function (response) {
                ...
                if (response.success && response.data.length > 0) {
                    response.data.forEach(criterio => {
                        const row = `<tr><td class="align-middle">${criterio.criterio_grupo}</td><td class="align-middle">${criterio.criterio_nome}</td><td class="align-middle">${criterio.criterio_unidade || ''}</td><td class="align-middle">${criterio.criterio_valor}</td><td class="text-center"><button class="btn btn-sm btn-warning btn-editar-criterio me-1" title="Editar"><i class="fas fa-pencil-alt"></i></button><button class="btn btn-sm btn-danger btn-excluir-criterio" data-id="${criterio.criterio_id}" title="Excluir"><i class="fas fa-trash"></i></button></td></tr>`;
        ...
        $('#ficha_produto_id').on('change', function () {
            ...
            $.post('ajax_router.php?action=getProdutoDetalhesParaFicha', { produto_id: produtoId, csrf_token: csrfToken }, function (response) {
                if (response.success) {
                    ...
                    const infoHtml = `<div class="row"><div class="col-md-6"><small class="text-muted">Denominação:</small><p class="fw-bold">${p.prod_classe || 'N/A'}</p></div><div class="col-md-3"><small class="text-muted">Marca:</small><p class="fw-bold">${p.prod_marca || 'N/A'}</p></div><div class="col-md-3"><small class="text-muted">NCM:</small><p class="fw-bold">${p.prod_ncm || 'N/A'}</p></div></div><div class="row mt-2"><div class="col-md-2"><small class="text-muted">Emb. Primária:</small><p class="fw-bold">${formatarPeso(p.peso_embalagem_primaria)}</p></div><div class="col-md-2"><small class="text-muted">Emb. Secundária:</small><p class="fw-bold">${formatarPeso(p.prod_peso_embalagem)}</p></div><div class="col-md-2"><small class="text-muted">Validade:</small><p class="fw-bold">${p.prod_validade_meses ? p.prod_validade_meses + ' meses' : 'N/A'}</p></div><div class="col-md-3"><small class="text-muted">EAN-13:</small><p class="fw-bold">${p.ean13_final || 'N/A'}</p></div><div class="col-md-3"><small class="text-muted">DUN-14:</small><p class="fw-bold">${p.prod_dun14 || 'N/A'}</p></div></div>`;
        ...
        $('#tbody-criterios').on('click', '.btn-excluir-criterio', function () {
            ...
            Swal.fire({ title: 'Tem certeza?', text: "Deseja realmente excluir este critério?", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'Cancelar', confirmButtonText: 'Sim, excluir!' }).then((result) => {
                if (result.isConfirmed) {
                    $.post('ajax_router.php?action=excluirCriterioFicha', { criterio_id: id, csrf_token: csrfToken }, (response) => {
                        if (response.success) {
        ...
        if ($('#modal-crop-image').length) {
                ...
                $.get(`ajax_router.php?action=listarFotosFicha&ficha_id=${fichaId}`, function (response) {
                    if (response.success && response.data.length > 0) {
                        response.data.forEach(foto => {
                            /* let idSufixo = foto.foto_tipo.toLowerCase().replace(/_/g, '-');
                             if (idSufixo.startsWith('embalagem-')) { idSufixo = idSufixo.replace('embalagem-', ''); }
                             if (idSufixo.startsWith('tabela-')) { idSufixo = idSufixo.replace('tabela-', ''); }*/
                            let idSufixo = '';
            ...
            $('#midia-pane').on('change', '.form-upload input[type="file"]', function () {
                ...
                if (file) {
                    ...
                    /*  let idSufixo = tipoFoto.toLowerCase().replace(/_/g, '-');
                      if (idSufixo.startsWith('embalagem-')) { idSufixo = idSufixo.replace('embalagem-', ''); }
                      if (idSufixo.startsWith('tabela-')) { idSufixo = idSufixo.replace('tabela-', ''); }*/
                    ...
                    currentUploadInfo = { fichaId, tipoFoto, form, previewImg: $(`#preview-${idSufixo}`) };
            ...
            $(modalCropElement).on('shown.bs.modal', () => {
                cropper = new Cropper(imageToCrop, { aspectRatio: NaN, viewMode: 1, autoCropArea: 0.9 });
            }).on('hidden.bs.modal', () => { if (cropper) { cropper.destroy(); cropper = null; } });
            ...
            $('#btn-crop-upload').on('click', function () {
                ...
                cropper.getCroppedCanvas({ width: 1024, imageSmoothingQuality: 'high' }).toBlob((blob) => {
                    ...
                    $.ajax({
                        url: 'ajax_router.php?action=uploadFotoFicha', type: 'POST', data: formData,
                        processData: false, contentType: false, dataType: 'json'
                    }).done((response) => {
                        if (response.success) {
                            Swal.fire({ icon: 'success', title: 'Sucesso!', text: 'Imagem enviada.', timer: 1500, showConfirmButton: false });
            ...
            $('#midia-pane').on('click', '.btn-remover-foto', function () {
                const fichaId = $('#ficha_id').val();
                const tipoFoto = $(this).data('tipo');
                Swal.fire({
                    title: 'Tem certeza?', text: "Deseja remover esta imagem?", icon: 'warning', showCancelButton: true,
                    confirmButtonColor: '#d33', cancelButtonText: 'Cancelar', confirmButtonText: 'Sim, remover!'
                }).then((result) => {
                    if (result.isConfirmed) {
                        $.post('ajax_router.php?action=excluirFotoFicha', { ficha_id: fichaId, foto_tipo: tipoFoto, csrf_token: csrfToken }, (response) => {
                            if (response.success) {
        ...
        if (fichaId) {
            $.post('ajax_router.php?action=getFichaTecnicaCompleta', { ficha_id: fichaId, csrf_token: csrfToken }, (response) => {
                if (response.success) { preencherFormularioFicha(response.data, false); }
            ...
            Swal.fire({ icon: 'info', title: 'Ficha Copiada', text: 'Selecione um novo produto antes de salvar.', showConfirmButton: true });
        }
    }
});