...
class EntidadeRepository
{
    ...
    public function findAllForDataTable(array $params): array
    {
        ...
        // Contexto da página (ex: se estou na tela de Clientes, pageType é 'cliente')
        ...
        switch (strtolower($pageType)) {
            case 'cliente':
                $pageConditions[] = "ent.ent_tipo_cliente = 1";
                break;
            case 'fazenda':
                $pageConditions[] = "ent.ent_tipo_fazenda = 1";
                break;
            case 'fornecedor':
                $pageConditions[] = "ent.ent_tipo_fornecedor = 1";
                break;
            case 'transportadora':
                $pageConditions[] = "ent.ent_tipo_transportadora = 1";
                break;
            default:
                $pageConditions[] = "1=1"; // Para listagem geral
        }
    ...
    public function create(array $data, int $userId): ?int
    {
        ...
        try {
            ...
            // --- TRATAMENTO DOS CAMPOS TINYINT (FLAGS) ---
            // Se o checkbox não estiver marcado, ele não vem no array $data, por isso usamos o isset
            $tipoCliente        = isset($data['ent_tipo_cliente']) ? 1 : 0;
            $tipoFornecedor     = isset($data['ent_tipo_fornecedor']) ? 1 : 0;
            $tipoFazenda        = isset($data['ent_tipo_fazenda']) ? 1 : 0;
            $tipoTransportadora = isset($data['ent_tipo_transportadora']) ? 1 : 0;
            ...
            $stmtEntidade->execute([
                ...
                ':tipo_fazenda'       => $tipoFazenda,
                ':tipo_transportadora' => $tipoTransportadora,
                ':situacao'           => $situacao,
    ...
    public function update(int $id, array $data, int $userId): bool
    {
        ...
        try {
            ...
            $sqlEntidade = "UPDATE tbl_entidades SET
                                ...
                                ent_tipo_cliente = :tipo_cliente,
                                ent_tipo_fornecedor = :tipo_fornecedor,
                                ent_tipo_fazenda = :tipo_fazenda,
                                ent_tipo_transportadora = :tipo_transportadora,
            ...
            if ($tipoFornecedor === 1) {
                ...
                // Se desmarcou "Fornecedor", removemos o vínculo
    ...
    public function getTransportadoraOptions(): array
    {
        $sql = "SELECT 
               ...
               -- WHERE ent_tipo_entidade = 'Transportadora' AND ent_situacao = 'A' 
                 WHERE ent_tipo_transportadora = 1 AND ent_situacao = 'A' 
                ORDER BY text ASC";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
